...
mussel_tree <- read.nexus("Unionida_tree.tre")
...
mussel_trees_20burned <- mussel_trees[24001:120001] #Remove burnin (20%)
mussel_trees_100 <- sample(mussel_trees_20burned, size=100) #Randomly select 100 trees
rm(mussel_trees)
rm(mussel_trees_20burned)
...
n= 32 # ~20% of total nº H-S associations
...
tangle.gram(TreeH, TreeS, HS, colscale= "diverging", colgrad=col.scale, 
            nbreaks=50, LFGD01, res.fq=TRUE, link.lwd=1, link.lty=1, fsize=0.4,
            pts=FALSE, link.type="straight", node.tag=TRUE,
            cexpt=1)
...
write.table(LFPACO01, "OE_Host_relationships.txt", sep = "\t")
dev.off()  # Turn off PDF
cmdstr = paste("open ", pdffn, sep="")
system(cmdstr) 
